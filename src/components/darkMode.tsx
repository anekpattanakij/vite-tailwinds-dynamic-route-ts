...
const Toggle = () => {
	const {darkMode, setDarkMode} = useContext(CommonContext).darkmode;
	...
	const { r, transform, cx, cy, opacity } =
		properties[darkMode === true ? 'sun' : 'moon'];
	...
	return (
		<button
			...
			onClick={onClick}
			type="button"
			title={`Enable ${darkMode === false ? 'dark' : 'light'} Mode`}>
			<animated.svg
				...
				stroke="currentColor"
				className="cursor-pointer text-dark-200 fill-dark-200 dark:text-light-50 dark:fill-light-50"
				...
				<animated.circle
					cx="12"
					cy="12"
					className="text-dark-200 fill-dark-200 dark:text-light-50 dark:fill-light-50"
					// @ts-expect-error React-Spring stuff
